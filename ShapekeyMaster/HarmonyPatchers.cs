...
namespace ShapeKeyMaster
{
    internal class HarmonyPatchers
    {
        ...
        private static Dictionary<ShapeKeyEntry, bool> ignoreFlag = new Dictionary<ShapeKeyEntry, bool>();
        ...
        private static void VerifyBlendValuesBeforeSet(ref TMorph __instance)
        {
            ...
            foreach (var shapeKeyEntry in Ui.SkDatabase.ShapeKeysByMorph(__instance))
            {
                ...
                {
#if (DEBUG)
					ShapeKeyMaster.pluginLogger.LogDebug($"Ingoring Key @ {__instance.bodyskin.body.maid.status.fullNameJpStyle} :: {__instance.Category} ::: {shapeKeyEntry.ShapeKey} :::: {shapeKeyEntry.ShapeKey}");
#endif
                ...
                {
#if (DEBUG)
					ShapeKeyMaster.pluginLogger.LogDebug($"Disabling Key @ {__instance.bodyskin.body.maid.status.fullNameJpStyle} :: {__instance.Category} ::: {shapeKeyEntry.ShapeKey} :::: {shapeKeyEntry.ShapeKey}");
#endif
                    __instance.BlendValues[index] = 0;
                    __instance.BlendValuesBackup[index] = 0;
                }
                else if (shapeKeyEntry.ConditionalsToggle && __instance.bodyskin != null && __instance.bodyskin.body &&
                         __instance.bodyskin.body.maid)
                {
#if (DEBUG)
					ShapeKeyMaster.pluginLogger.LogDebug($"Applying Conditional State @ {__instance.bodyskin.body.maid.status.fullNameJpStyle} :: {__instance.Category} ::: {shapeKeyEntry.ShapeKey} :::: {shapeKeyEntry.ShapeKey}");
#endif

                    // When the condition is not met, skip the processing of shapekey, so other programs can use this shape key
                    if (SlotChecker.CheckIfSlotDisableApplies(shapeKeyEntry, __instance.bodyskin.body.maid))
                    {
                        if (ignoreFlag.ContainsKey(shapeKeyEntry) && ignoreFlag[shapeKeyEntry] == true)
                        {
                            continue;
                        }
                    ...
                    {
                        ignoreFlag[shapeKeyEntry] = false;

                        if (shapeKeyEntry.AnimateWithExcitement &&
                            YotogiLvls.Contains(SceneManager.GetActiveScene().name))
                        {
                ...
                {
#if (DEBUG)
					ShapeKeyMaster.pluginLogger.LogDebug($"Applying Excitement Deformation @ {__instance.bodyskin.body.maid.status.fullNameJpStyle} :: {__instance.Category} ::: {shapeKeyEntry.ShapeKey} :::: {shapeKeyEntry.ShapeKey}");
#endif
                ...
                {
#if (DEBUG)
					ShapeKeyMaster.pluginLogger.LogDebug($"Applying Standard Deform @ {__instance.bodyskin.body.maid.status.fullNameJpStyle} :: {__instance.Category} ::: {shapeKeyEntry.ShapeKey} :::: {shapeKeyEntry.ShapeKey}");
#endif
